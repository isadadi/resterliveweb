  <script type="text/javascript">
    ...
    $('#form-ini3').ajaxForm({
    	success: function(data){
    		$('.bs-modal-lg-delete').modal("toggle");
    		 ...
    		 $("#ajax").load("<?=base_url('libur/ajax_libur')?>");
    	}
    });
    ...
    $('.edit-fak').on('click',function(){
    	var id = $(this).parent().find('.id-fak').val();    	
    	$.post("<?=base_url('libur/get_libur/')?>"+id,function(data){    		
    		$('#tgl_mulai').prop("value",data['libur_tgl_mulai']);
    		$('#tgl_selesai').prop("value",data['libur_tgl_selesai']);
        $('#keterangan').prop("value",data['libur_keterangan']);
        $('#libur_id').prop("value",data['libur_id']);
